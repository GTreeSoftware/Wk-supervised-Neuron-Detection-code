"""This function is used to build the architecture of the VoxResNet model, more detailed explanation can be referred
   to the paper "VoxResNet: Deep voxelwise residual networks for brain segmentation from 3D MR images" by Chen in 2018"""
import torch
import torch.nn as nn

# build the residual unit in the model
class Res_Module(nn.Module):
    ...
    # set the initial parameters for the residual model
...
# this class is used to build the Batch_Normalization and Activation function
...
# this class is used to build the architecture of VoxResNet
class VoxResNet_3D(nn.Module):
    def __init__(self,in_ch,out_ch,act_fn=nn.ReLU(inplace=True)):
        ...
        # you can choose to use dropout or not. Not used now
    ...
    # set the initial parameters of the network
    def reset_params(self):
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                nn.init.kaiming_normal_(m.weight)
...
# the main function is used to test the VoxResNet
if __name__=="__main__":
    ...
    # output the network parameters for testing
    params = list(model.parameters())
    k = 0
    for i in params:
        l = 1
        print("the layer of the architecture：" + str(list(i.size())))
        for j in i.size():
            l *= j
        print("the parameters number of the layer：" + str(l))
        k = k + l
